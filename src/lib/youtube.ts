...
async function getRecentVideoIdsForChannel(channelId: string): Promise<string[]> {
...
export const getLiveStreamsForStreamers = cache(
  async (streamers: Streamer[]): Promise<LiveStream[]> => {
    ...
    const recentIdsByChannel = await mapWithConcurrency(
      ...
      async (streamer) => {
        try {
          const ids = await getRecentVideoIdsForChannel(streamer.channelId);
    ...
    const videoItems = (
      await mapWithConcurrency(videoChunks, 2, async (ids) => {
        try {
          const data = await youtubeFetchJson<YoutubeVideosResponse>("videos", {
